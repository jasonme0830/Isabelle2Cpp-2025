...
void
Code::bind_cons(const string& cons, Datatype& datatype)
{
  cons_types_.emplace(cons, datatype);
}

Datatype*
Code::find_datatype_by_cons(const string& name)
{
...
void
Code::add_short_def(const string& name, Ptr<ShortDef> short_def)
{
  short_defs_.emplace(name, move(short_def));
}

const ShortDef*
Code::get_short_def(const string& name) const
{
...
vector<reference_wrapper<const Datatype>>
Code::datatypes() const
{
...
vector<reference_wrapper<const FuncEntity>>
Code::func_entities() const
{
