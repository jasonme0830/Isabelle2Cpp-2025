...
struct OptimizerOption
{
  ...
  bool reduce_cond; // removes the conditions for the last pattern for total
                    // functions (not use option `nonexhaustive`)
  bool use_deque; // use std::deque instead of std::list as the target type for
                  // list

  // experimental options that do not improve performance
  bool use_class; // generates classes instead of structs for datatypes
  bool uncurry;   // returns lambda-expression if the arguments are less than
