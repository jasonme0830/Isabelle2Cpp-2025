...
namespace hol2cpp {
void
TypeInference::pattern_infer(Ptr<Expr>& expr, const string& funcname)
{
  if (typeid(*expr) == typeid(VarExpr)) {
    VarExpr& arg_trans = reinterpret_cast<VarExpr&>(*expr);
    if (the_cons_type_mapping.find(arg_trans.name) !=
        the_cons_type_mapping.end())
      return;

    the_arg_type_mapping.emplace(arg_trans.name, ref(arg_trans.expr_type));
  ...
  // IntegeralExpr has been given NormalType by the function arglist (nat, int,
  // <N word>?)
}

void
TypeInference::pattern_consexpr_infer(ConsExpr& consexpr,
                                      const string& funcname)
{
  // all ConsExpr in pattern is datatype's constructor (Cons x xs) :: nat list
  DatatypeDef& dtypedef =
    (the_dtypedef_mapping.find(consexpr.constructor))->second.get();
  auto component = find_if(dtypedef.components.begin(),
                           dtypedef.components.end(),
                           [cons = consexpr.constructor](auto& com) {
  ...
  if (typeid(*dtypedef.decl_type) == typeid(NormalType)) {
    for (size_t i = 0; i < consexpr.args.size(); ++i) {
      ...
      VarExpr& consexpr_arg_trans =
        reinterpret_cast<VarExpr&>(*consexpr.args[i]);
      the_arg_type_mapping.emplace(consexpr_arg_trans.name,
    ...
    TemplateType& trans_expr_type =
      reinterpret_cast<TemplateType&>(*consexpr.expr_type);
    // trans_decl_type includes all argumentType args
    TemplateType& trans_decl_type =
      reinterpret_cast<TemplateType&>(*dtypedef.decl_type);
    ...
    for (size_t i = 0; i < trans_decl_type.args.size(); ++i) {
      try {
        ArgumentType& argument_type_trans =
          dynamic_cast<ArgumentType&>(*trans_decl_type.args[i]);
        the_argument_type_ins_mapping.emplace(argument_type_trans.name,
                                              trans_expr_type.args[i]->clone());
      } catch (bad_cast&) {
    ...
    for (size_t i = 0; i < component->arguments.size(); ++i) {
      if (typeid(*consexpr.args[i]) == typeid(VarExpr)) {
        if (typeid(*component->arguments[i]) == typeid(ArgumentType)) {
          ...
          ArgumentType& com_arg_trans =
            reinterpret_cast<ArgumentType&>(*component->arguments[i]);
          auto& ins_type =
            the_argument_type_ins_mapping.find(com_arg_trans.name)->second;
          consexpr.args[i]->expr_type = ins_type->clone();

          try {
            VarExpr& consexpr_arg_trans =
              dynamic_cast<VarExpr&>(*consexpr.args[i]);
            the_arg_type_mapping.emplace(consexpr_arg_trans.name,
                                         ref(consexpr_arg_trans.expr_type));
          } catch (bad_cast&) {
            cerr << "pattern_consexpr_infer(): bad cast ArgumentType.\n";
        ...
        ConsExpr& consexpr_trans =
          reinterpret_cast<ConsExpr&>(*consexpr.args[i]);
...
void
TypeInference::pattern_list_or_set_infer(Expr& expr,
                                         const std::string& funcname)
{
  if (typeid(expr) == typeid(ListExpr)) {
    ...
    // trans_expr_type includes the institution type(s)
    TemplateType& trans_expr_type =
      reinterpret_cast<TemplateType&>(*expr_trans.expr_type);

    //[]
    if (trans_expr_type.args.size() == 0)
      return;
    ...
    for (size_t i = 0; i < expr_trans.exprs.size(); ++i) {
      ...
      if (typeid(*expr_trans.exprs[i]) == typeid(VarExpr)) {
        VarExpr& list_subexpr_trans =
          reinterpret_cast<VarExpr&>(*expr_trans.exprs[i]);
        the_arg_type_mapping.emplace(list_subexpr_trans.name,
                                     ref(list_subexpr_trans.expr_type));
      } else if (typeid(*expr_trans.exprs[i]) == typeid(ConsExpr)) {
        ConsExpr& list_subexpr_trans =
          reinterpret_cast<ConsExpr&>(*expr_trans.exprs[i]);
        pattern_consexpr_infer(list_subexpr_trans, funcname);
      } else if (typeid(*expr_trans.exprs[i]) == typeid(ListExpr) ||
                 typeid(*expr_trans.exprs[i]) == typeid(SetExpr)) {
    ...
    for (size_t i = 0; i < expr_trans.exprs.size(); ++i) {
      expr_trans.exprs[i]->expr_type = ins_type->clone();
      if (typeid(*expr_trans.exprs[i]) == typeid(VarExpr)) {
        VarExpr& set_subexpr_trans =
          reinterpret_cast<VarExpr&>(*expr_trans.exprs[i]);
        the_arg_type_mapping.emplace(set_subexpr_trans.name,
                                     ref(set_subexpr_trans.expr_type));
      } else if (typeid(*expr_trans.exprs[i]) == typeid(ConsExpr)) {
        ConsExpr& set_subexpr_trans =
          reinterpret_cast<ConsExpr&>(*expr_trans.exprs[i]);
