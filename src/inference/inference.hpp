...
public:
    ...
    void analy_from_bottom(Expr &expr, const std::string &funcname, LambdaMap &lambda_types_map, LambdaMap &return_lambda_types_map);
...
private:
    ...
    */
    std::map<std::string, std::reference_wrapper<DatatypeDef>> the_dtypedef_mapping;

    /**
     * map constructor's name to its construct rule which has been transformed into FuncType:
     * e.g. <Cons, ('a => 'a list => 'a list)>, <Nil, 'a list>...
    */
