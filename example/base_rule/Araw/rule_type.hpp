...
template<typename T1>
std::deque<T1> ModifyValue(const T1 &arg1, const T1 &arg2, const std::deque<T1> &arg3) {
    // ModifyValue a b [] = []
    if (arg3.empty()) {
        return std::deque<T1>();
    }
...
template<typename T1>
std::deque<T1> ModifyIndex(const std::uint64_t &arg1, const T1 &arg2, const std::deque<T1> &arg3) {
    // ModifyIndex n b [] = []
    if (arg3.empty()) {
        return std::deque<T1>();
    }
...
template<typename T1>
std::uint64_t slength(const std::deque<T1> &arg1) {
    // slength [] = 0
    if (arg1.empty()) {
        return 0;
    }
...
template<typename T1>
std::deque<T1> stake(const std::uint64_t &arg1, const std::deque<T1> &arg2) {
    // stake 0 xs = []
    if (arg1 == 0) {
        return std::deque<T1>();
    }
...
template<typename T1>
std::deque<T1> sdrop(const std::uint64_t &arg1, const std::deque<T1> &arg2) {
    // sdrop 0 xs = xs
    if (arg1 == 0) {
        auto xs = arg2;
        return xs;
...
template<typename T1>
T1 snth(const std::deque<T1> &arg1, const std::uint64_t &arg2) {
    // snth [] n = 0
    if (arg1.empty()) {
        return 0;
    }
