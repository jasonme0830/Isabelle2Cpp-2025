...
std::list<T1> AddListTail(T1 arg1, std::list<T1> arg2) {
    ...
    auto a = arg1;
    auto x = arg2.front();
    auto xs = std::list<T1>(arg2.begin() + 1, arg2.end());
...
std::list<T1> AddListI(std::uint64_t arg1, T1 arg2, std::list<T1> arg3) {
    ...
    if (arg1 == 0) {
        if (!arg3.empty()) {
            auto a = arg2;
            auto x = arg3.front();
            auto xs = std::list<T1>(arg3.begin() + 1, arg3.end());
            auto temp0 = xs;
            temp0.push_front(x);
...
std::list<T1> DelListHead(std::list<T1> arg1) {
    // DelListHead (x#xs) = xs
    if (!arg1.empty()) {
        auto xs = std::list<T1>(arg1.begin() + 1, arg1.end());
        return xs;
    }
...
std::list<T1> DelListTail(std::list<T1> arg1) {
    ...
    // DelListTail (Cons a []) = []
    if (!arg1.empty()) {
        if (std::list<T1>(arg1.begin() + 1, arg1.end()).empty()) {
            return std::list<T1>();
        }
    }
...
std::list<T1> DelListI(std::uint64_t arg1, std::list<T1> arg2) {
    ...
    // DelListI i (Cons a []) = []
    if (!arg2.empty()) {
        if (std::list<T1>(arg2.begin() + 1, arg2.end()).empty()) {
            return std::list<T1>();
        }
    }
...
template<typename T1>
std::list<T1> Modify1(T1 arg1, T1 arg2, std::list<T1> arg3) {
    // Modify1 a b [] = []
    if (arg3.empty()) {
        return std::list<T1>();
    }
...
template<typename T1>
std::list<T1> Modify2(std::uint64_t arg1, T1 arg2, std::list<T1> arg3) {
    // Modify2 n b [] = []
    if (arg3.empty()) {
        return std::list<T1>();
    }
...
std::list<T1> app(std::list<T1> arg1, std::list<T1> arg2) {
    ...
    auto a = arg1.front();
    auto as = std::list<T1>(arg1.begin() + 1, arg1.end());
    auto bs = arg2;
...
std::list<T1> Merge(std::list<T1> arg1, std::list<T1> arg2) {
    ...
    auto xs = std::list<T1>(arg1.begin() + 1, arg1.end());
    auto y = arg2.front();
    auto ys = std::list<T1>(arg2.begin() + 1, arg2.end());
    std::list<T1> temp0;
    if (x <= y) {
        auto temp1 = ys;
        temp1.push_front(y);
...
std::list<T1> MergeSort(std::list<T1> arg1) {
    ...
    auto xs = arg1;
    return Merge(MergeSort(std::list<T1>(xs.begin(), xs.begin() + size(xs) / 2)), MergeSort(std::list<T1>(xs.begin() + size(xs) / 2, xs.end())));
}
