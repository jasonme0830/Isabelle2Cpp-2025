...
template<typename T1>
std::list<T1> Modify1(const T1 &arg1, const T1 &arg2, const std::list<T1> &arg3) {
    // Modify1 a b [] = []
    if (arg3.empty()) {
        return std::list<T1>();
    }
...
template<typename T1>
std::list<T1> Modify2(const std::uint64_t &arg1, const T1 &arg2, const std::list<T1> &arg3) {
    // Modify2 n b [] = []
    if (arg3.empty()) {
        return std::list<T1>();
    }
...
std::list<T1> MergeSort(const std::list<T1> &arg1) {
    ...
    auto xs = arg1;
    auto temp1 = xs;
    auto temp0 = MergeSort(std::list<T1>(xs.begin(), xs.begin() + size(std::move(temp1)) / 2));
    auto temp3 = xs;
