...
template<typename T1>
std::deque<T1> butlast(std::deque<T1> arg1) {
    // butlast [] = []
    if (arg1.empty()) {
        return std::deque<T1>();
    }
...
template<typename T1>
std::deque<T1> filter(const std::function<bool(const T1 &)> &arg1, std::deque<T1> arg2) {
    // filter P [] = []
    if (arg2.empty()) {
        return std::deque<T1>();
    }
...
T1 foldl(const std::function<T1(const T1 &, const T2 &)> &arg1, const T1 &arg2, std::deque<T2> arg3) {
    // foldl f a [] = a
    if (arg3.empty()) {
        return arg2;
    }
...
template<typename T1>
std::deque<T1> takeWhile(const std::function<bool(const T1 &)> &arg1, std::deque<T1> arg2) {
    // takeWhile P [] = []
    if (arg2.empty()) {
        return std::deque<T1>();
    }
...
template<typename T1>
std::deque<T1> dropWhile(const std::function<bool(const T1 &)> &arg1, std::deque<T1> arg2) {
    // dropWhile P [] = []
    if (arg2.empty()) {
        return std::deque<T1>();
    }
...
template<typename T1, typename T2>
std::deque<std::pair<T1, T2>> zip(std::deque<T1> arg1, std::deque<T2> arg2) {
    // zip xs [] = []
    if (arg2.empty()) {
        return std::deque<std::pair<T1, T2>>();
    }
    ...
    auto temp0 = ([&] {
        ...
        auto z = temp1.front();
        temp1.erase(temp1.begin(), temp1.begin() + 1);
...
template<typename T1>
std::optional<T1> find(const std::function<bool(const T1 &)> &arg1, std::deque<T1> arg2) {
    // find _ [] = None
    if (arg2.empty()) {
...
template<typename T1>
std::uint64_t count_list(std::deque<T1> arg1, const T1 &arg2) {
    // count_list [] y = 0
    if (arg1.empty()) {
        return 0;
    }
...
template<typename T1>
std::deque<T1> remove1(const T1 &arg1, std::deque<T1> arg2) {
    // remove1 x [] = []
    if (arg2.empty()) {
        return std::deque<T1>();
    }
...
template<typename T1>
std::deque<T1> removeAll(const T1 &arg1, std::deque<T1> arg2) {
    // removeAll x [] = []
    if (arg2.empty()) {
        return std::deque<T1>();
    }
...
template<typename T1>
bool distinct(std::deque<T1> arg1) {
...
template<typename T1>
std::deque<T1> remdups(std::deque<T1> arg1) {
    // remdups [] = []
    if (arg1.empty()) {
        return std::deque<T1>();
    }
...
template<typename T1>
std::deque<T1> remdups_adj(std::deque<T1> arg1) {
    // remdups_adj [] = []
    if (arg1.empty()) {
        return std::deque<T1>();
    }
...
template<typename T1>
std::deque<T1> replicate(const std::uint64_t &arg1, const T1 &arg2) {
    // replicate 0 x = []
    if (arg1 == 0) {
        return std::deque<T1>();
    }
...
template<typename T1>
std::deque<std::pair<std::uint64_t, T1>> enumerate(const std::uint64_t &arg1, std::deque<T1> arg2) {
...
template<typename T1>
std::deque<T1> rotate1(std::deque<T1> arg1) {
    // rotate1 [] = []
    if (arg1.empty()) {
        return std::deque<T1>();
    }
...
template<typename T1>
std::deque<T1> splice(std::deque<T1> arg1, std::deque<T1> arg2) {
    // splice [] ys = ys
    if (arg1.empty()) {
        return arg2;
    }
...
template<typename T1, typename T2>
T1 arg_min_list(const std::function<T2(const T1 &)> &arg1, std::deque<T1> arg2) {
    // arg_min_list f [x] = x
    if (arg2.size() == 1) {
        auto x = arg2[0];
        return x;
    ...
    auto x = arg2.front();
    auto y = arg2[1];
    arg2.erase(arg2.begin(), arg2.begin() + 2);
...
template<typename T1, typename T2>
std::deque<T1> insort_key(const std::function<T2(const T1 &)> &arg1, const T1 &arg2, std::deque<T1> arg3) {
    // insort_key f x [] = [x]
    if (arg3.empty()) {
        return std::deque<T1>{arg2};
    }
...
template<typename T1>
std::pair<std::deque<T1>, std::deque<T1>> partition(const std::function<bool(const T1 &)> &arg1, std::deque<T1> arg2) {
    // partition P [] = ([], [])
    if (arg2.empty()) {
        return std::make_pair(std::deque<T1>(), std::deque<T1>());
    }
    ...
    std::pair<std::deque<T1>, std::deque<T1>> temp1;
    if (arg1(x)) {
...
template<typename T1, typename T2>
std::deque<T2> map_tailrec_rev(const std::function<T2(const T1 &)> &arg1, std::deque<T1> arg2, std::deque<T2> arg3) {
...
template<typename T1, typename T2>
std::deque<T2> map_tailrec(const std::function<T2(const T1 &)> &arg1, std::deque<T1> arg2) {
    // map_tailrec f as = rev (map_tailrec_rev f as [])
    return rev(map_tailrec_rev(arg1, std::move(arg2), std::deque<T2>()));
}
...
template<typename T1>
std::uint64_t gen_length(const std::uint64_t &arg1, std::deque<T1> arg2) {
