...
std::list<T1> InsertSort(const std::list<T1> &arg1) {
    ...
    auto xs = arg1;
    return InsertSortPart(std::move(xs), std::list<T1>());
}
...
tree<T1> copy_tree(const tree<T1> &arg1) {
    ...
    if (arg1.is_Node()) {
        auto left = arg1.as_Node().p1().self();
        auto x = arg1.as_Node().p2();
        auto right = arg1.as_Node().p3().self();
...
bool searchtree1(const T1 &arg1, const tree<T1> &arg2) {
    ...
    auto a = arg1;
    auto left = arg2.as_Node().p1().self();
    auto x = arg2.as_Node().p2();
    auto right = arg2.as_Node().p3().self();
...
tree<T1> changetree(const T1 &arg1, const T1 &arg2, const tree<T1> &arg3) {
    ...
    auto a = arg1;
    auto b = arg2;
    auto left = arg3.as_Node().p1().self();
    auto x = arg3.as_Node().p2();
    auto right = arg3.as_Node().p3().self();
    tree<T1> temp2;
    if (a == x) {
        auto temp3 = tree<T1>::Node(
            left,
