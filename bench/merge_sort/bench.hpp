...
#include "merge_sort_raw.hpp"

#include <cassert>
#include <benchmark/benchmark.h>

static void bench_merge_sort_raw(benchmark::State &state) {
...
static void bench_merge_sort_raw_ud(benchmark::State &state) {
    auto length = state.range(0);
    ...
    for (auto _ : state) {
        auto nlist = merge_sort_r_ud(list);
...
BENCHMARK(bench_merge_sort_raw)->RangeMultiplier(2)->Range(256, 16384);
BENCHMARK(bench_merge_sort_raw_ud)->RangeMultiplier(2)->Range(256, 16384);
